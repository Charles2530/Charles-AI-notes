...
- 文本提示词：用自然语言描述主体、风格、场景和约束。
- 类别标签：用离散类别控制生成对象或类别条件。
- 边缘图或深度图：用结构信号约束轮廓、几何和空间布局。
- 参考图像：提供身份、风格、构图或局部外观锚点。
- 布局框：指定对象大致位置和画面版式。
...
- 文本编码：把 prompt 变成模型可读取的语义条件。
- 图像特征：从参考图、首帧或局部区域中提取外观与身份信息。
- 结构图：用边缘、深度、姿态或分割图约束空间形状。
- 动作或视频上下文：为视频、动作生成或机器人场景提供时间条件。
...
- 指定内容：生成主体、风格、场景或类别要贴合用户意图。
- 结构约束：布局、姿态、边缘、深度或分割不能随意漂移。
- 输入保留：身份、构图、背景或未编辑区域需要保持一致。
- 局部编辑：在已有图像上修改目标区域，而不是完全从零生成。
...
- 扩散模型方向：给出“样本应如何变得更自然”的去噪方向。
- 分类器方向：给出“样本应如何更像类别 \(c\)”的条件梯度。

两者叠加后，样本会更朝目标条件靠拢。

**优点**：

- 条件信号明确：分类器梯度直接指向目标类别，解释性强。

**缺点**：

- 需要额外分类器：每个噪声水平下都要有可用的条件判别模型。
- 高分辨率或复杂条件实现麻烦：文本、布局、姿态等条件很难都交给普通分类器。
...
- \(\epsilon_\theta(x_t,t,\varnothing)\)：无条件分支，表示不看条件时的自然去噪方向。
- \(\epsilon_\theta(x_t,t,c)\)：条件分支，表示看条件 \(c\) 后的去噪方向。
- \(s\)：guidance scale，控制条件偏移被放大的强度。
...
- image-to-image：以输入图为起点，在保留结构的同时重绘外观。
- inpainting：只重绘 mask 区域，未遮挡区域尽量保持不变。
- 风格迁移：保留内容或布局，改变材质、画风或视觉风格。
- 参考图生成：从参考图继承身份、角色、商品或构图线索。
...
- 直接拼接输入通道：把结构图或 mask 与 noisy latent 一起输入，适合强空间条件。
- cross-attention：让图像 latent 读取文本或其他条件 token，适合语义控制。
- adaptive normalization：用条件改变归一化尺度和偏置，常用于风格或类别调制。
- control branch：增加专门处理结构条件的分支，如 ControlNet 风格设计。

**从直觉上看**：

- 低层注入：更影响边缘、纹理、局部位置和精细结构。
- 高层注入：更影响主体类别、构图意图和全局语义。
...
- 保留主模型能力：原有生成先验、风格和语义能力不必从头训练。
- 增加控制分支：把边缘、深度、姿态等结构条件交给专门路径处理。

**这样可以让模型同时保住**：

- 图像生成能力：画面质量、语义丰富度和基础审美不被破坏。
- 结构可控性：姿态、轮廓、深度或布局能稳定约束输出。
...
- 高 guidance 场景下 `DPM-Solver++` 价值更明显：它对 guided diffusion ODE 的稳定性更友好。
- 低阶 solver 在极强 guidance 时更容易发散：一阶近似难以跟上被放大的陡峭轨迹。
...
- negative prompt：告诉模型应远离哪些属性、风格或伪影。
- 多参考图：同时提供身份、风格、姿态、材质等多个锚点。
- 局部区域条件：只在 mask 或指定区域内施加强约束。
...
- 条件优先级：文本、结构、参考图和 mask 哪个约束更强要明确。
- 冲突覆盖规则：当参考图要保身份、文本要改风格时，需要定义谁让步。
- 层级作用范围：不同条件应影响局部纹理、空间结构还是全局语义。
