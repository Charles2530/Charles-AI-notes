...
- 单步噪声强度：每个 timestep 往样本里加入多少随机扰动。
- 信号衰减速度：前期是否过快丢掉结构，后期是否还留有可学习的细节。
- 梯度可用性：不同噪声区间是否还能给模型提供稳定监督。
...
- 实现简单：只需按固定区间线性设置 \(\beta_t\)，教学和复现都方便。
- 早期使用广泛：DDPM 等经典实现常从线性日程出发，便于对照基线。

**缺点**：

- SNR 分布不总是理想：线性 \(\beta_t\) 不等于线性信息难度，某些噪声段可能过密或过稀。
- 少步或特定分辨率下未必最优：压缩采样步数时，日程偏差会更容易放大成结构或细节问题。
...
- 训练更平衡：SNR 衰减更平滑，模型不容易只偏向某一段噪声水平。
- 少步采样更稳：稀疏选 timestep 时，轨迹通常不那么极端。
...
- ODE/SDE 统一：用连续噪声尺度描述训练和采样，更容易接到概率流与随机微分方程视角。
- sampler 设计更直接：沿 \(\sigma\) 区间选步长、排采样点和配 solver 会更清楚。
...
- 实现简单：目标就是回归注入的 \(\epsilon\)，监督构造直接。
- 训练稳定：目标尺度清晰，许多基础实现和经验都围绕它建立。
- 与 DDPM 主线一致：经典 DDPM 教材和代码通常默认使用这种参数化。
...
- 损失尺度不同：同一预测误差换到不同目标坐标后，数值大小和敏感区间会变。
- 梯度权重不同：不同 timestep 对参数更新的贡献会随参数化改变。
- guidance 与少步适配不同：某些坐标系在强条件引导或极少步 solver 下更容易稳定。
...
- 少步采样兼容性更好：压缩步数时不容易只偏向噪声或原图一端。
- guided generation 更稳：强 guidance 下轨迹尺度更平衡，过冲风险较低。
- latent space 里更舒服：latent diffusion 中信号和噪声尺度差异较大，\(v\) 坐标常更好优化。
...
- 低噪声细节权重：更强调边缘、纹理和局部保真。
- 高噪声语义权重：更强调从弱信号中恢复主体结构和全局语义。
...
- 噪声日程：决定训练样本落在哪些噪声区间，以及 SNR 如何变化。
- 参数化：决定模型用 \(\epsilon\)、\(x_0\)、score 还是 \(v\) 这样的坐标来学习。
- loss weighting：决定不同噪声区间的误差在优化里有多重要。
- sampler：决定推理时如何把学到的向量场离散成有限步更新。
...
- 训练稳定：loss、梯度和样本质量不要在某些噪声段突然崩掉。
- 高频细节：纹理、边缘、小物体和局部清晰度要能保住。
- 文本对齐：生成内容需要贴合 prompt，而不是只生成自然但跑题的图。

**这里往往更在意**：

- 合理噪声日程：让语义、结构和细节区间都有足够学习压力。
- 与 guidance 兼容的参数化：强条件引导时不容易颜色过冲、结构断裂或局部发散。
...
- 与 solver 的匹配：训练坐标和采样更新公式要配合，否则低步数会暴露偏差。
- 极少步下的误差放大：4 步、8 步甚至一步时，单次预测错误会被直接放大到成图质量里。
...
- 多帧一致性：身份、纹理和动作轨迹在时间上不能跳变。
- 时间噪声分配：噪声不仅要沿生成步分配，还要考虑帧间结构是否同步。
- 条件控制稳定性：文本、轨迹、首帧或参考图条件需要跨帧保持影响。
