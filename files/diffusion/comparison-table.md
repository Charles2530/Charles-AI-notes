...
- 随机反向链是否唯一：DDPM 的随机采样路径不是唯一可行生成路径。
- 能否换成确定轨迹：DDIM 构造更平滑、可控、适合少步的采样路径。
...
- 分布匹配稳定性：学生分布要接近教师或数据分布，训练梯度不能太噪。
- 模式坍塌风险：学生不能只生成平均化、安全但单调的样本。
- 一步映射保真：构图、语义和局部纹理都要在极少更新中保住。
...
- `DDIM`：改变采样路径，不改训练模型。
- `Euler`：低成本一阶 ODE/SDE 离散化。
- `Heun`：带校正的二阶采样更新。
- `DPM-Solver` / `DPM-Solver++`：利用扩散 ODE 结构的专用少步求解器。

**特点是**：

- 现成模型可用：无需重训即可切换采样器或 schedule。
- 适合第一层优化：上线前可以快速试质量、速度和稳定性。
- 风险低且易回退：效果不佳时恢复原采样器即可。
...
- Progressive Distillation：逐级把多步教师压成少步学生。
- Consistency / LCM：学习不同时间点输出一致结果的少步映射。
- DMD 系列：用分布匹配目标追求一步或极少步生成。
- Rectified Flow / Diffusion：训练更直的生成路径，降低少步积分难度。

**特点是**：

- 训练换推理：离线多花训练成本，线上减少网络评估次数。
- 蒸馏越重线上越轻：学生模型承担更短采样链。
- 依赖教师、目标和数据：教师质量、蒸馏损失和覆盖范围会决定上限。

### 5.3 训练和推理都更复杂

某些复杂条件生成或视频生成场景下，方法虽然理论上更快，但整个系统变得更复杂：

- CFG 变种：更强条件控制会改变采样轨迹和数值稳定性。
- ControlNet/adapter 条件注入：额外条件分支增加前向开销和显存压力。
- 多尺度 latent 级联：不同分辨率或阶段之间还要协调接口和误差。
- 视频时空注意力：帧间建模会显著增加 token 数和 cache 成本。

这时“采样步数减少”未必能直接换来端到端吞吐提升，因为瓶颈可能转移到：

- UNet / DiT 单次前向成本：步数少了，但每步网络仍可能很重。
- 条件网络开销：ControlNet、adapter 或文本/图像编码器会成为新瓶颈。
- 显存带宽：低步数不一定减少权重和激活读写压力。
- 视频 cache 成本：联合多帧建模会让中间状态和注意力缓存迅速变大。
