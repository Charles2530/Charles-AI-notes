...
- 一段自然语言回答：把视觉证据组织成可读解释或问答结果。
- 一个分类标签：把图像、图文对或局部区域归到预定义类别。
- 一组目标框 \((b_i, c_i)\)：同时给出位置和类别，服务检测、grounding 或 UI 操作。
- 一串操作指令：把视觉理解转成工具调用、屏幕动作或机器人高层步骤。
...
- 检索高效：图像向量和文本向量可直接做相似度搜索。
- 图文库可以预编码：离线先建好向量索引，在线只算查询侧。
- 工程简单：适合大规模召回、商品搜索和粗粒度匹配。

**缺点**：

- 交互粒度粗：通常只看全局向量相似度，难精确对齐局部区域。
- 不适合复杂多轮问答：问题与图像细节需要反复交互时能力不足。
...
- 图文匹配：判断一段文本和一张图是否描述同一内容。
- VQA：根据问题动态读取相关视觉 token 后生成答案。
- grounding：把文本短语对齐到图像中的框、点或区域。
...
- 可以复用成熟 LLM：语言生成、指令遵循和工具格式能力不用从零训练。
- 训练成本相对可控：冻结大部分模块时，只需重点训练视觉连接器和少量适配层。
- 易于迁移到对话、工具使用、代理任务：输出形式能沿用 LLM 的自回归接口。

**缺点**：

- projector 太弱会丢细节：视觉 token 被压缩过度后，小字、坐标和局部关系进不了 LLM。
- LLM 不天然擅长空间定位：语言能力强不等于能稳定输出精确框、点位或布局关系。
...
- 视觉读取：先从图像 token 中找到与任务有关的证据。
- 问题理解：再判断文本真正关心的字段、对象、关系或动作。
- 序列生成：最后按语言或结构化格式逐 token 输出结果。
...
- 视觉问答：训练模型把图片证据转成针对问题的答案。
- OCR 问答：强化小字、数字、表格字段和版面文本读取能力。
- 屏幕操作轨迹：让模型学习界面元素、点击目标和多步流程。
- 图像推理链：提供中间证据或解释，减少只凭语言先验作答。
...
- 图文检索：让文本查询能找到匹配图像，或反向用图搜文本。
- 向量召回：把大规模候选库先压到可重排的小集合。
- rerank 前的粗筛：用低成本相似度过滤明显无关候选。

**配方**：

- 双塔结构：图像和文本分别编码，方便离线建库和在线检索。
- 大规模对比学习：用海量正负对塑造共享 embedding 空间。
- 尽量大 batch：提供更多 in-batch negatives，提高对比学习分辨率。
...
- 看图问答：根据视觉内容和用户问题生成答案。
- 文档理解：读取页面文字、表格、版式和跨页关系。
- 屏幕代理：理解 UI 并输出点击、输入或工具调用步骤。

**配方**：

- 冻结或半冻结 LLM：复用已有语言能力，降低训练成本和灾难性遗忘。
- 视觉编码器 + projector：把视觉 token 映射到 LLM 可消费的表示空间。
- 大量视觉指令微调：让模型学会按用户任务组织多模态输出。
...
- 票据解析：抽取金额、日期、税号、商户等结构化字段。
- 表格理解：保持行列、表头、合并单元格和数值关系。
- 长文档问答：跨页检索证据并组织答案。

**配方**：

- 高分辨率视觉输入：保留小字、表格线和局部印章等细节。
- OCR 文本与版面位置信息联合建模：同时利用文字内容和页面坐标。
- 局部区域裁剪与全局页面共同输入：兼顾细节识别和整体版式关系。
...
- 局部病灶敏感性：不能漏掉小范围异常区域。
- 术语生成能力：报告需要符合医学表达和结构化规范。
- 否定表达准确性：要正确区分“有异常”和“未见异常”。
...
- 检索变强但回答变空泛：全局 embedding 更好，却缺少细粒度生成监督。
- 回答流畅但定位变差：语言目标压过了 grounding 目标。
- OCR 准确但开放式推理下降：模型会读字，但跨区域组合证据的能力没跟上。
