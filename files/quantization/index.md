...
- 权重显存：模型参数本身占用的显存，决定能否装进目标硬件。
- KV cache：长上下文和高并发时的动态显存大户。
- 带宽：权重、激活和 cache 反复读写会限制 token 生成速度。
- 推理吞吐：单位时间能服务多少 token 或请求，直接影响成本。
...
- 文档理解：小字和结构化字段对数值误差更敏感。
- 图表与表格：坐标、图例、行列关系和数字一致性不能乱。
- 屏幕代理：按钮定位和 UI 文本识别受视觉精度影响很大。
- 视频理解：长时序一致性和跨帧记忆会放大量化误差。
- 机器人动作：动作平滑性和安全边界对误差容忍度更低。
...
- GPTQ：用近似二阶信息做后训练权重量化，重点降低权重量化误差。
- AWQ：利用激活统计保护重要权重通道，常用于低比特权重量化。
- SmoothQuant：把激活离群值压力迁移到权重，改善权重和激活同时量化。
- FP8：用低精度浮点格式换取硬件友好的训练或推理吞吐。
- 混合精度服务：按层、张量或模块选择不同精度，平衡质量和性能。
