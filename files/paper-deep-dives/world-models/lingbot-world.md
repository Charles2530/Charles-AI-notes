...
!!! note "图解：长时记忆图在看不可见对象是否还存在"
    这张图对应世界模型里很关键的一点：模型不能只生成下一段视频，还要在看不见目标的时间里维持隐藏状态。静态地标离开视野后仍应保持空间结构，桥梁一段时间不可见后距离关系仍应合理，车辆离开当前视野后也应继续沿物理上合理的轨迹运动。否则 agent 一转身，场景就被“重新生成”，规划器就无法依赖它做长期决策。
