...
![Diffusion Policy input output](../assets/images/paper-figures/vla/diffusion-policy-input-output.jpg){ width="920" }
...
- 借用语言模型训练框架：离散动作 token 可以直接接自回归 loss、teacher forcing 和序列并行工具链。
- 多模态统一更自然：图像 token、语言 token 和动作 token 更容易放进同一序列建模框架。

**缺点**：

- 量化误差影响平滑性：连续动作被分桶后，执行轨迹可能出现台阶感或抖动。
- 精细装配不一定够：插接、旋拧、微小力控等任务常需要更高分辨率动作。
...
- 前 3 步接近把手：先把末端执行器移动到可抓取位置。
- 中间 2 步闭合夹爪：在对准后完成抓握，而不是边偏移边夹。
- 后 3 步后拉：保持抓握并沿抽屉打开方向平滑移动。
...
- 手眼相机图像：从末端附近看目标，适合精细抓取和接触判断。
- 场景第三视角图像：提供全局布局、障碍物和任务上下文。
- 深度图：补充几何距离，帮助判断高度、遮挡和可达性。
- 末端位姿：记录夹爪或工具当前在工作空间中的位置和朝向。
- 关节角：描述机器人自身状态，是运动学和安全约束的基础输入。
- 力觉或触觉：感知接触、滑动和阻力，弥补纯视觉盲区。
...
- “盘子已经插进去了”：力和视觉都显示状态稳定，可以进入下一步。
- “盘子被挡住了还在外面”：位置接近但插入失败，需要重新对准。
- “盘子边缘刮到金属架，应该轻微调整姿态”：接触力异常，说明要改变角度而不是继续硬推。
...
- 多峰动作分布更友好：同一观测下可以保留多条合理动作模式，而不是被 MSE 平均掉。
- 多种抓法可表达：从左侧、右侧或不同接近角度完成任务时，模型能生成其中一种可行动作。
...
- 抓偏后的恢复：需要重新观察、松开、微调位置，而不是继续执行原轨迹。
- 遮挡时的处理：判断该暂停观察、移动视角，还是换抓取路径。
- 拉不动抽屉的判断：区分没抓住、方向不对、卡住或需要更大但安全的力。
...
- 玻璃架反光：视觉特征容易混淆，需要更稳的深度或多视角证据。
- 前排物品遮挡后排空间：路径规划不能只看目标物，还要看可达通道。
- 夹爪不能碰倒鸡蛋盒：动作必须考虑脆弱物体和安全边界。
...
- 按钮很小：需要高分辨率视觉和精确末端定位。
- 面板布局不同：模型不能只记固定位置，要理解当前界面结构。
- 干扰按钮很多：目标识别和点击动作都必须避免邻近误触。
- 误按代价高：执行前最好有二次确认或安全约束。
...
- 观测表示足够完整：视觉、状态和接触信息能覆盖任务关键变量。
- 动作表示足够稳定：输出既可执行，又不把连续控制变成抖动轨迹。
- 轨迹建模能表达多峰分布：同一任务存在多种合理动作时，不会学成平均动作。
- 数据经过坐标和任务语义统一：不同平台、相机和指令能被放到共同接口里。
- 推理闭环能及时纠正：小误差出现后能重新观察、调整和恢复，而不是一路累积。
