...
!!! note "初学者先抓住"
    ...
    容量规划先看请求画像，再看资源。平均输入长度、平均 QPS 都不够用；少量超长文档、长输出、工具 agent 或多模态请求，可能吃掉大部分 GPU 时间和 KV 显存。

!!! note "难点解释：为什么优化要按请求桶做"

    短聊天的瓶颈可能是 queue 和 launch 开销，长文档的瓶颈可能是 prefill 和 KV，长输出的瓶颈可能是 decode，agent 的瓶颈可能在工具。把它们混成一个平均数，会让团队选择错误优化动作。
