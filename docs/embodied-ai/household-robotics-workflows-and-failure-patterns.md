...
下面这张图把家庭任务拆成“指令理解、找物定位、路径规划、抓取操作、完成验证、收尾复盘”六段。家庭机器人最难的不是每段都永远不失败，而是失败发生后能否澄清、换视角、重规划、重试或安全接管。

![家庭机器人任务与失败恢复流程](../assets/images/embodied-ai/generated/embodied-household-task-recovery-workflow.png){ width="920" }

**读图提示**：评测家庭机器人时，不要只看单步抓取成功率。真正影响可用性的是长任务中断后的恢复能力，以及机器人是否会把小失败扩大成更大的混乱。
