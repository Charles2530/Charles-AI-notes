...
下面这张图把任务层级和评测桶放在一起看。它的重点是：不同具身任务需要的指标不同，不能用一个“成功率”覆盖所有任务。低层控制更关心误差、平滑和接触质量；长时任务更关心恢复、记忆和安全边界；人机协作还要看用户体验和可预期性。

![具身任务谱系与评测桶](../assets/images/embodied-ai/generated/embodied-task-benchmark-buckets.png){ width="920" }

**读图提示**：Benchmark 会塑造研究方向。如果评测没有覆盖恢复能力、接触质量和真实长尾风险，模型可能在榜单上进步，却在部署中退化。
