...
## 补充：把 **数据质量、去重与治理** 当成训练平台的一部分

训练页面很容易被写成方法综述，但真正把模型训起来时，**数据质量、去重与治理** 通常并不是一个孤立决策，而是和数据、并行、恢复、评测、成本一起联动的。更实用的读法是先把它放回训练平台语境中：

1. **lineage、去重粒度、污染审计、许可风险、版本化**。

一旦缺少这种平台视角，团队就容易把很多跨层问题都简化成“改超参”“加数据”或者“再堆 compute”。这在小实验里也许还能蒙对，在大规模训练、长上下文、多模态和后训练阶段几乎一定会反复返工。

## 补充：更稳妥的设计与试验顺序

围绕 **数据质量、去重与治理**，建议把设计过程拆成四步：
...
围绕 **数据质量、去重与治理**，最常见的失败并不一定是训练直接爆炸，而是更隐蔽的几类：
...
更稳的 **数据质量、去重与治理** 实践，至少应当满足下面几条：

1. **建立 lineage**。
2. **把污染审计常态化**。
3. **分开治理训练浪费与 benchmark 泄漏**。
4. **快照版本化**。
...
只有这样，**数据质量、去重与治理** 才会真正沉淀成组织能力，而不是停留在某次实验里的一段经验。
